...
###update 3-23-23 after meeting with Doug:
  
#1. PlotID should never be a random effect. Each plot is specific to a treatment. So plot 1 in gr does
  #not correspond to plot 1 in te. (One could block up the treatments, but it would take some time.)
#2. For sample 1 (first sample of the year), te is an appropriate measure of productivity. But ANP must
  #be measured at the daily level because intervals vary, so divde by # days. IE:
    #daily productivity = (te biomass of sample 1) / # dat$days
#3. For samples 2-5, to calculate productivity, must subtract gr value of previous time period from te
    #BUT: sometimes there was no 'te' sample, because there were no herbivores present (esp. in '88-'89)
    #in this case, it is assumed consumption is 0 and ANP = gr of t2 minus gr of t1
#4. So, algorithm is:
  #1. Sample 1: ANP = te
  #2. Samples 2+:
    #take sample of time t
    #is there a te treatment? if yes, ANP is te of time t minus gr of time t-1 (mean difference two populations)
      #if no, assume that gr is without consumption; make consumption = 0, ANP is gr time t minus gr time t-1
#5. Once beta functions are fit to each site and year, get posteriors of site peak biomass X and Y
#6. Model time of peak biomass according to elevation and soil C; talk to Doug
